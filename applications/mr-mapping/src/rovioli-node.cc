...
void RovioliNode::saveMapAndStartNewMap(
    const std::string& path, const bool overwrite_existing_map,
    bool process_to_localization_map) {
  std::unique_ptr<MapBuilderFlow> prev_map_builder_flow = map_builder_flow_.release();
  if (FLAGS_rovioli_run_map_builder) {
    map_builder_flow_.reset(
            new MapBuilderFlow(
                    camera_system, std::move(maplab_imu_sensor), save_map_folder));
  ...
  if (prev_map_builder_flow) {
    prev_map_builder_flow->saveMapAndOptionallyOptimize(
